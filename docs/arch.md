...
**当前阶段目标**: 复现 Lua 5.5.0 核心语义 + 关键系统扩展
- 以 Lua 5.5.0 的 VM、调用、闭包、元方法、错误语义作为核心基线
- 保留并发展 AQL 容器系统、类型系统、class/self/method 和 AI 原生语法扩展
- AQL 扩展必须显式标记，不能隐式改变 Lua 5.5.0 已定义的核心语义
- 为后续AI原生特性提供高性能运行时基础
