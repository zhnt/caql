...
AQL VM集成测试框架旨在系统性地测试`avm_core.c`的所有功能，确保VM核心行为对齐 Lua 5.5.0，同时覆盖 AQL 容器等显式扩展。
...
# 运行特定类别测试
./bin/vmtest test/vm/bytecode/basic bin/aqlvm

# 运行单个测试
./bin/vmtest test/vm/bytecode/basic bin/aqlvm -t add

# 详细模式
./bin/vmtest test/vm/bytecode/basic bin/aqlvm -v
```
